...
class LoopState():
    ...
    def accessInLoop(self, point, access_time):
        ...
        if access_time <= access_threshold:
            if self.stat_res.get(dis):
                self.stat_res[dis].append(access_time)
            else:
                self.stat_res[dis] = [access_time]
...
def multi_main(text_regions):
    ...
    around = r'D:\ma_data\accessibility'
    ...
    access_data = gpd.read_file(around_poly_shp)
    # commute_data = pd.read_csv(r'D:\ma_data\通勤率\全量通勤率\{}.txt'.format(text_regions))
    for i, r in tqdm(access_data.iterrows(), position=0, desc="city", leave=False, colour='red', ncols=80):
        ls.accessInLoop(point=[r['X'], r['Y']], access_time=r["grid_code"])
    ...
    #                                                       index=None)
    pd.DataFrame({'dis': dis, 'avg(access-time)': com}).sort_values(by='dis').to_csv(
        os.path.join(r"D:\ma_data\通勤率\可达性", text_regions + '.csv'),
        index=None)

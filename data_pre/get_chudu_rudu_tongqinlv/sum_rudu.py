...
def main(regions):
    ...
    print(g_r.regions)
    # ['上海市', '乌鲁木齐市', '北京市', ..., '青岛市']
    for region in tqdm(g_r.regions, position=0, desc="city", leave=False, colour='red', ncols=80):
        print(region)
...
def multi_main(regions):

    namecode = FromNameToCode()
    # ['上海市', '乌鲁木齐市', '北京市', ..., '青岛市']
    for region in tqdm(regions, position=0, desc="city", leave=False, colour='red', ncols=80):
        print(region)
        adcodes = namecode.nameTocode(region[:-1])
        fntca = FromNameToCoreAroundShp(region=region)
        fntca_core = fntca.core_poly
            ...
            try:
                ...
                for i, r in tmp_data.iterrows():
                    if Point(r['lon1'], r['lat1']).intersects(fntca_around) and Point(r['lon2'], r['lat2']).intersects(
                            fntca_around):
                        if region_around_p_dict.get(str(r['lon2']) + '*' + str(r['lat2'])):
                            region_around_p_dict[str(r['lon2']) + '*' + str(r['lat2'])].append(
                                [r['lon1'], r['lat1'], r['cnt']])
                        else:
                            region_around_p_dict[str(r['lon2']) + '*' + str(r['lat2'])] = [
                                [r['lon1'], r['lat1'], r['cnt']]]
            except:
                pass
...
if __name__ == '__main__':
    g_r = GetRegions(r'D:\ma_data\数据处理\core')
    already_rs = ["贵阳市", "济南市" , "沈阳市", "郑州市"]
    text_regions = [[tmp] for tmp in g_r.regions if tmp not in already_rs]

    p = multiprocessing.Pool()
    p.map(multi_main, text_regions)
    # main()
